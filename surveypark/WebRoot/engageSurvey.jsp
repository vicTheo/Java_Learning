			...
			<table>
							...
							<s:iterator var="q" value="currPage.questions">
								...
								<tr>
									<td class="tdOptionArea">
										<!-- 判断当前题型是否属于前四种 -->
										<s:if test='#qt lt 4'>
											<s:iterator var="option" value="#q.optionsArr" status="st">
										...
										<s:if test="#qt == 4">
											<select name='q<s:property value="#qId" />'>
												<s:iterator var="option" value="#q.optionsArr" status="optst">
													<option value='<s:property value="#optst.index" />'
										...
										<s:if test='#qt> 5'>
											<table>
												...
												<tr>
													<td></td>
													<!-- 循环输出列表标题数组 -->
													<s:iterator var="col" value="#q.matrixColTitlesArr">
														<td><s:property value="#col" /></td>
													</s:iterator>
												</tr>
												<!-- 循环输出每一行,对行标题数组进行遍历 -->
												<s:iterator var="row" value="#q.matrixRowTitlesArr" status="rowst">
													<tr>
														<td><s:property value="#row" /></td>
														<!-- 循环输出控件 -->
														<s:iterator var="col" value="#q.matrixColTitlesArr" status="colst">
															<td>
																<!-- 矩阵式单选按钮 -->
																<s:if test="#qt == 6">
																	<input type="radio" 
																...
																<s:elseif test="#qt == 8">
																	<select name='q<s:property value="#qId"/>'>
																		<s:iterator var="option" value="#q.matrixSelectOptionsArr" status="optst">
				...
				<tr>
					<td colspan="2" align="center">
						<!-- 构造上一步按钮 -->
						<s:if test="currPage.orderNum != #session.current_survey.minOrderNum">
							<input type="submit" name='submit_pre' value='<s:property value="#session.current_survey.preText"/>' class="btn">
						</s:if>
						<!-- 构造下一步按钮 -->
						<s:if test="currPage.orderNum != #session.current_survey.maxOrderNum">
